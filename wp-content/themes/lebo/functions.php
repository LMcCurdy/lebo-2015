...
// Fix ® and ™ in titles
add_filter( 'the_title', 'tm_reg_post_title');
function tm_reg_post_title ($title) {
	if(!is_admin()){
		$find = array('®','™');
		$rplc = array('<sup>®</sup>','<sup>™</sup>');
		$title = str_replace($find,$rplc,$title);
	}
	return $title;
}
...
function my_acf_load_value($value)
{
	if(!is_admin()){
		$find = array('®','™');
		$rplc = array('<sup>®</sup>','<sup>™</sup>');
		$value = str_replace($find,$rplc,$value);
	}
    return $value;
}
add_filter('acf/load_value/name=procedure', 'my_acf_load_value');
...
add_image_size( 'featured', 1200, 540, true );
add_image_size( 'sq280', 280, 280, true );
...
function bones_register_sidebars() {
    ...
    register_sidebar(array(
    'id' => 'giveback',
    'name' => __( 'Giveback', 'bonestheme' ),
    'description' => __( 'The sidebar for the Give Back page.', 'bonestheme' ),
    'before_widget' => '<div id="%1$s" class="widget %2$s">',
    'after_widget' => '</div>',
    'before_title' => '<h3 class="widgettitle">',
    'after_title' => '</h3>',
   ...
   register_sidebar(array(
    'id' => 'testimonials',
    'name' => __( 'Testimonials', 'bonestheme' ),
    'description' => __( 'The sidebar for the testimonials page.', 'bonestheme' ),
    'before_widget' => '<div id="%1$s" class="widget %2$s">',
    'after_widget' => '</div>',
    'before_title' => '<h3 class="widgettitle">',
    'after_title' => '</h3>',
...
function exclude_widget_categories($args) {
    // $exclude = "4,18,6,8,10,9,5"; // The IDs of the excluding categories
...
// get summary
/* use: <p><?php echo getSummary(get_the_excerpt(),165); ?></p> */
function getSummary($str, $length) {
    ...
    if (!empty($pos)) {
     ...
     return html_entity_decode($orig);
    }
}
...
function catch_that_image() {
  ...
  if(empty($first_img)) {
    $first_img =  get_template_directory_uri()."/assets/images/default_image.jpg";
  }
  return $first_img;
}
...
function left_func($atts, $content = null) {
  $a = shortcode_atts( array(
    'class' => ''
  ), $atts);
    $output = do_shortcode($content);
    return '<div class="left-shortcode'.($a["class"] ? ' '.$a["class"] : '').'">' . $output . '</div>';
}
...
function right_func($atts, $content = null) {
  $a = shortcode_atts( array(
    ...
    $output = do_shortcode($content);
    return '<div class="right-shortcode'.($a["class"] ? ' '.$a["class"] : '').'">' . $output . '</div>';
}
